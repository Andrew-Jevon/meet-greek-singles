...
//        }
	...
	*/
    ...
    function loadImageFromString($imageString)
    {
	    if ( $this->image ) {
	        imagedestroy($this->image);
	    }
        ...
        if(!($this->image = imagecreatefromstring($imageString))) {
            return false;
        }

        if(!($this->image_type = $this->getImageTypeFromString($imageString))) {
            return false;
        }
	...
	*/
    ...
    static function getImageFromString($imageString)
    {
        $image = false;

        if($imageString) {
            if(preg_match("/^data:image\/(?<type>(?:png|gif|jpg|jpeg));base64,(?<image>.+)$/", $imageString, $matchings)){
    ...
    static function getImageTypeFromString($imageString)
    {
        $imageType = false;

        if($imageString) {
            if(preg_match("/^data:image\/(?<type>(?:png|gif|jpg|jpeg));base64,(?<image>.+)$/", $imageString, $matchings)){
                $imageType = base64_decode($matchings['type']);
            }
        }
