...
class CustomPage extends CHtmlBlock{
    ...
    const TABLE_COL_ORDER = 'col_order';
    ...
    static function checkAccessPage($id = null)
    {
        ...
        $page = DB::select(self::getTable(), '`id` = ' . to_sql($id) . ' OR `parent` = ' .  to_sql($id));
    ...
    static function parseMenu(&$html, $section = 'narrow', $dbIndex = DB_MAX_INDEX, $where = '')
    {
        ...
        if ($section == 'narrow') {
            ...
            $menuSystem = array(
                ...
                'menu_bottom_search_results' => array(
                            'url' => Common::pageUrl('search_results'),
                ...
                if ($item['system']) {
                    ...
                    if ($alias == 'column_narrow_rated_your_photos') {
                        ...
                        if(!Common::isMobile(true)) {
                            ...
                            if(!guid() || $detect->isMobile() || !trim(Common::getOption('facebook_appid'))) {
                                continue;
                            }
                        }
                    }
...
//        $page = DB::rows($sql);
        $item = self::loadPageInfo($id, $g['lang_loaded']);
        DB::setFetchType(MYSQL_BOTH);
        ...
        $html->setvar('url_page_for_history', Common::pageUrl('page') . '?id=' . $id);
    }

    static public function loadPageInfo($id, $language)
    {
        $where = '(`lang` = "default" AND `id` = ' . to_sql($id) . ')
                    OR (`lang` = ' . to_sql($language) . ' AND `parent` = ' . to_sql($id) . ')';
        $pageInfo = DB::one(self::getTable(), $where, '`id` DESC');
        return $pageInfo;
    }

    static public function getTitle($id, $language)
    {
        $pageInfo = self::loadPageInfo($id, $language);
    ...
    static public function setTableColOrder()
    {
        self::setTable(self::TABLE_COL_ORDER);
    }

    static public function getMenuNamePostfix()
    {
