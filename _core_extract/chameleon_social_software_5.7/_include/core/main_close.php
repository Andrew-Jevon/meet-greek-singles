...
if (isset($page)) {
	...
	if (IS_DEMO && $sitePart === 'administration') {
        $demoActionPagesAvailable = array(
            'index.php',
            'fakes_reply_mails.php',
            'fakes_reply_im.php',
            'fakes_reply_winks.php',
            'fakes_friend_requests.php',
            'fakes_reply_replier.php',
        );
		if (in_array($p, $demoActionPagesAvailable) || ($p == 'users_edit.php' && get_param('cmd') == 'location')) {
            $page->action();
        }
	} else {
		$page->action();
