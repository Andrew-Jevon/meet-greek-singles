var CWall = function() {
    ...
    this.itemAdd = function() {
        ...
        var post = $this.inputPostAdd[0].innerHTML;
        post = post.replace(/[\r\n]/g, '');
        ...
        var imageFromDrawer = ImageDrawer.getImage('wall_post');

        if(!post&&!uploadImageLoaded&&!imageFromDrawer){
...
//console.log($itemTemp.outerHeight(), $itemTemp.outerHeight(true),$itemTemp[0].offsetHeight, $itemTemp[0].clientHeight);
    ...
    this.commentAdd = function(inp) {
        ...
        var imageDrawerElementId = 'wall_comment_' + ($(inp).hasClass('wall_comment_area_top') ? 'top_' : '') + id;
        if($(inp).hasClass('btn_comment_send')) {
            imageDrawerElementId = $(inp).siblings('.icon_drawer').attr('id');
        }
		...
		if (!sticker) {
			...
			if(!comment && !audioMessageId && !uploadImageLoaded && !imageFromDrawer){
    ...
    this.commentAddReplies = function(inp) {
        ...
        var imageFromDrawer = ImageDrawer.getImage('wall_comment_reply_' + cid);
		...
		if (sticker) {
            ...
            ImageDrawer.close('wall_comment_reply_' + cid);
			$inp.text('').trigger('autosize',fnSubmit).focus();
		}
        return false;
	}
