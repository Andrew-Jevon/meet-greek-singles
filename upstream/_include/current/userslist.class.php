...
class Users_List {
    ...
    static function show($where = '', $order = '', $from_add = '', $template = '', $from_group = '', $redirectIfSingle=false)
    {
        ...
        if (Wall::isProfileWall($display)
            ...
            // 2026-05-04: match-score sort per Irene's spec.
            // For logged-in viewers with completed onboarding, prepend a
            // compatibility-score expression to ORDER BY (referencing the
            // userinfo alias `i` from search_results.php's FROM clause).
            // We sort by the expression directly — NOT the alias `match_score`
            // — because the ionCube-encoded CUsers core on production does
            // not always append fieldsFromAdd to SELECT, which caused
            // "Unknown column 'match_score' in ORDER BY" when clicking the
            // sidebar logo / vanity profile URLs. Badge values are still
            // filled client-side via /match_scores.php (see _list_users_info).
            if (guid() && $p == 'search_results.php' && $display !== 'rate_people' && $display !== 'encounters') {
                $msFile = dirname(__FILE__) . '/match_score.class.php';
                if (is_readable($msFile)) {
                    include_once($msFile);
                    if (class_exists('MatchScore') && MatchScore::userHasAnswers(guid())) {
                        $scoreSql = MatchScore::buildScoreSql(guid());
                        if ($scoreSql !== '') {
                            $orderByScore = '(' . $scoreSql . ') DESC';
                            $order = $order ? ($orderByScore . ', ' . $order) : $orderByScore;
                        }
                    }
                }
            }
            ...
            $isWall = (guid() || $optionTmplName == 'edge')
                        && Common::isWallActive()
                        && $display == 'profile'
