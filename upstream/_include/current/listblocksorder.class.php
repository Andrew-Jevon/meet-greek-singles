...
class ListBlocksOrder extends CHtmlBlock {
	...
	static $numberItemProfileMenuBig = 6;
    ...
    static function getIconSvg($type){
        if(!$type){
            return '';
        }
    ...
    function action(){
        ...
        if($cmd == 'update') {
            ...
            $highlighted = get_param('highlighted');
                ...
                if(self::$module == 'member_header_menu') {
                    foreach ($order as $key => $item) {
                        ...
                        $listItems[$item] = array(
                            'active' => isset($status[$item]) ? 1 : 0,
                            'highlighted' => isset($highlighted[$item]) ? 1 : 0,
                        );
                    }
                }
    ...
    static function getOrderItemsList($module, $onlyActive = true, $tmpl = null, $admin = false)
    {
        global $sitePart;
        global $p;
        ...
        if ($tmpl == 'edge') {
            ...
            if ($module == 'main_page_block_order') {
                ...
                if($module !== 'member_column_left_order') {
                    unset($itemsList['photos_private']);
                }
				...
				if (LiveStreaming::notAviableLiveStreamingAdmin()) {
                    ...
                    unset($itemsList['menu_live_list_finished_edge']);
				}
        ...
        if ($onlyActive) {
            foreach($orderItemsList as $k => $v){
                if (!$v['active'] || (isset($v['active']['active']) && !$v['active']['active'])) {
    ...
    static function parseMenuItem(&$html, $data, $block, $k, $prf = '', $checkCounter = false, $j = 0)
    {
        ...
        $html->subcond($block == 'member_header_menu' && (isset($data['active']['highlighted']) && $data['active']['highlighted']), "{$blockItem}_highlighted");
    ...
    static function parseAdditionMenu(&$html, $module = '', $block = '', $maxVisibleItem = 6)
    {
        ...
        $isParseMore = false;
        $isParseMoreItem = $uid != $guid || $module == 'member_groups_additional_menu';
    ...
    static function parseListAdmin(&$html)
    {
        ...
        foreach($orderItemsList as $k => $v){
            ...
            if(($module != 'member_header_menu' && $v['active']) || (isset($v['active']['active']) && $v['active']['active'])){
            ...
            if($module == 'member_header_menu') {
                $highlightedChecked = (isset($v['active']['highlighted']) && $v['active']['highlighted']) ? 'checked' : '';
                $html->setvar('order_item_highlighted_checked', $highlightedChecked);
                $html->parse('order_item_highlighted', false);
            }
