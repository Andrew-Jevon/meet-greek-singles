...
Class TemplateEdge {
    ...
    static $listUsersMainPage = false;
        ...
        if ($guid) {
            if ($isHeader) {
                ...
                $varsHeader = array(
                    ...
                    'edge_general_settings_list_photos_number_items' => Common::getOptionInt('list_photos_number_items', 'edge_general_settings'),
            ...
            if ($isFooter) {
                ...
                Common::parseDrawerBlock($html, 'media_comment_top_drawer');
                Common::parseDrawerBlock($html, 'media_comment_bottom_drawer');
            }
            ...
            if ($p == 'index.php') {
                ...
                if ($html->blockExists($blockInfoHeader)) {
					...
					if ($isHeaderShort) {
                        ...
                        self::parseFeaturedUsersOnMainPage($html);
            ...
            if ($html->blockExists($block)) {
                if ($key == 'color_scheme' || $key == 'color_scheme_general' ||
                    ...
                    if ($block == 'color_scheme_visitor') {
                        ...
                        $colorSchemeOptions['list_people_featured_border_color'] = Common::getOption('list_people_featured_border_color', 'edge_main_page_settings');
                        $colorSchemeOptions['list_people_featured_border_style'] = ($colorSchemeOptions['list_people_featured_border_color'] === 'transparent' ? 'none' : 'solid');
    ...
    static function isTemplateColums($showAlways = false)
    {
        ...
        $isProfile = ($p === 'search_results.php' && get_param('display') === 'profile');

        return ($guid || $isProfile) && Common::isAllowedModuleTemplate('profile_column')
    ...
    static function parseColumnListImg(&$html, $uid, $type, $numColumn, $typeOrder, $blockColumnType, $groupId = 0)
    {
        ...
        $isPhotos = in_array($type, array('photos', 'photos_private', 'photos_list_1', 'photos_list_2'));
        //$isBlogs = in_array($type, array('photos', 'photos_list_1', 'photos_list_2'));
        if ($isPhotos) {
            $private = ($type === 'photos_private') ? true : false;
            $list = CProfilePhoto::getPhotosList($typeOrder, false, '0, ' . $numColumn, $uid, $groupId, false, $private);
            $count = CProfilePhoto::getTotalPhotos($uid, false, $groupId, $private);
        } else {
        ...
        if (in_array($type, array('photos_list_1', 'photos_list_2', 'videos_list_1', 'videos_list_2'))) {
            ...
            if ($groupId) {
                ...
                if ($groupsPhotoList && $uid == $guid) {
                    ...
                    } elseif ($type == 'photos_private') {
                        $url = Common::pageUrl('user_photos_private_list');
                    } elseif($type == 'videos') {
            ...
            if (in_array($type, array('photos', 'photos_private', 'videos')) && guid() != $uid) {
        ...
        foreach ($list as $id => $item) {
            if ($type == 'photos' && $item['private'] == 'Y') {
                    continue;
            } elseif ($type == 'photos_private' && $item['private'] == 'N') {
                    continue;
            } elseif ($type == 'photos_private' && $item['private'] == 'Y') {
                if(!User::hasPrivatePhotoAccess($guid, $uid)) {
                    $item['description'] = '';
                }
            }
    ...
    static function parseColumn(&$html, $uid = null, $row = null, $showAlways = false)
    {
        ...
        if ($isPageVids || $isPagePhotos) {
            ...
            if ($isPageVids) {
                ...
                unset($blocksColumnLeft['photos_private']);
            } else {
        ...
        foreach ($blocksColumnLeft as $type => $value) {
            ...
            $blockItem = '';
            if (($type == 'photos' || $type == 'photos_private' || $type == 'videos') && $parseBlockPhotos) {
    ...
    static function getUsersListMainPage()
    {
        if(self::$listUsersMainPage === false) {
            ...
            $numberFeaturedUsers = self::getFeaturesUsersOnMainPageCount();
            $numberUsersAll = $numberUsers + $numberFeaturedUsers;

            self::$listUsersMainPage = User::listUsers($typeOrder, $numberUsersAll);
        }

        return self::$listUsersMainPage;
    }
    ...
    static function parseListUsersMainPage(&$html, $blockItems = 'main_page_list_people')
    {
        ...
        $rows = self::getUsersListMainPage();
        if ($rows) {
            foreach ($rows as $row) {
    ...
    static function parseListPhotos(&$html, $typeOrder, $limit, $groupId = 0, $showAllMyPhoto = false, $private = false)
    {
        $layoutType = 'default';
        $rows = CProfilePhoto::getPhotosList($typeOrder, false, $limit, null, $groupId, $showAllMyPhoto, $private);
    ...
    static function parsePhoto(&$html, $row, $numberRow, $layoutType)
    {
        ...
        if ($html->blockExists($blockItemType)) {
            ...
            if($row['private'] == 'Y' && ($guid != $uidParam && !User::isFriend($guid, $uidParam))) {
                $row['description'] = '';
            ...
            if ($uidParam && $uidParam == $guid && $row['user_id'] == $guid) {
                ...
                if($row['private'] === 'Y') {
                    if(!$row['group_id']) {
                        $html->parse("{$blockItemType}_make_public", false);
                    }
                } else {
                    if(!$row['group_id']) {
                        $html->parse("{$blockItemType}_make_private", false);
                    }
                    $html->parse("{$blockItemType}_hide_header", false);
                    $html->parse("{$blockItemType}_make_profile_picture", false);
                }
    ...
    static function parseFeaturedUsersOnMainPage(&$html)
    {
        if(!self::isFeaturedUsersOnMainPageActive()) {
            return;
        }

        $type = Common::getOption('list_people_featured_style', 'edge_main_page_settings');
        $usersLimit = self::getFeaturesUsersOnMainPageCount();

        $block = 'info_block_header_visitor_featured_users';

        if($usersLimit && $html->blockExists($block)) {

            $rows = self::getUsersListMainPage();
            if ($rows) {

                if(!Common::isOptionActive('info_block_header_show', 'edge_main_page_settings')) {
                    $html->setvar($block . '_width', 'main-page-profiles-full-page');
                }

                $usersForModule = self::getFeaturesUsersOnMainPageCount();

                $blockProfile = $block . '_profile';
                $profileNumber = 0;
                foreach ($rows as $row) {
                    $profileNumber++;
                    $html->setvar($blockProfile . '_type', $type);
                    $html->setvar($blockProfile . '_number', $profileNumber);
                    $html->setvar($blockProfile . '_url', User::url($row['user_id'], $row));
                    $html->setvar($blockProfile . '_name', toAttr(User::getInfoBasic($row['user_id'], 'name')));
                    $html->setvar($blockProfile . '_photo', uphoto($row['user_id'], $type === 'circle' ? 'b' : 'm'));
                    $html->parse($blockProfile);

                    array_shift(self::$listUsersMainPage);

                    if($profileNumber >= $usersLimit) {
                        break;
                    }
                }
                $html->parse($block);
    ...
    static function getFeaturesUsersOnMainPageCount()
    {
        $usersCount = 0;

        if(self::isFeaturedUsersOnMainPageActive()) {
            if(Common::getOption('list_people_featured_style', 'edge_main_page_settings') === 'circle') {
                $usersCount = 5;
            } else {
                $usersCount = intval(trim(Common::getOptionInt('list_people_featured_number', 'edge_main_page_settings')));
            }
        }
    ...
    static function isFeaturedUsersOnMainPageActive()
    {
        return (Common::isOptionActive('list_people_featured', 'edge_main_page_settings')) && (Common::getOption('info_block_leayout', 'edge_main_page_settings') === 'info_block_right');
    }
}