...
class CHeader extends CHtmlBlock {
    ...
    static function parseApp(&$html)
    {
        ...
        if($html->blockexists($blockMobileApp) && Common::isAppIos()) {
            $iosAppVersion = Common::iosAppVersion();
            if($iosAppVersion !== '5.4' && preg_match('/\d\.\d/', $iosAppVersion)) {
                $html->setvar('ios_app_version', '-' . $iosAppVersion);
            }
        ...
        if($html->blockexists($blockMobileApp) && Common::isAppAndroid()) {
            ...
            if($androidAppVersion !== '5.3' && preg_match('/\d\.\d/', $androidAppVersion)) {
    ...
    function parseBlockImpact(&$html)
    {
        ...
        if(!guid() && !in_array($p, array('join2.php', 'about.php', 'contact.php', 'page.php', 'info.php', 'search_results.php'))
        ...
        //if ($guid) {
        if ($p !== 'join2.php' && !in_array($p, $noAllowPages)) {
            ...
            if ($guid && $html->blockexists($blockMenu)) {
            ...
            if ($html->blockExists($blFooterMember)) {
                ...
                if(!$guid) {
                    $html->parse('footer_member_right_visitor');
                }
                ...
                if ($p != 'city.php') {
                    if($guid) {
                        $html->parse('footer_member_right_user');
                    }
...
*/
            ...
            if ($optionTmplSet == 'urban' && $this->name != 'impact_mobile') {
                if (guid()){
                    if (($p == 'profile_view.php' && $display != 'profile_info')
                        || ($p == 'messages.php' && $display == 'one_chat')
                        || !in_array($p, array('upgrade.php', 'profile_settings.php', 'profile_view.php', 'profile_personal_edit.php', 'messages.php', 'page.php'))) {
        ...
        if ($html->varExists('url_logo')) {
            $html->setvar('url_logo', guid() ? Common::getHomePage() : Common::pageUrl('index'));
        }
        ...
        if(guid()
            || $this->name != 'impact'
            || ($this->name == 'impact' && in_array($p, array('about.php', 'contact.php', 'page.php', 'info.php', 'search_results.php')))) {
            if ($this->name != 'edge') {
        ...
        */
            ...
            if (guid()) {
                ...
                if ($html->blockexists($blockHeaderUser)) {
                    ...
                    if ($p == 'search_results.php') {
                        ...
                        $pageTitle = CustomPage::getTitle(get_param('id'), $g['lang_loaded']);
                    }
        ...
        if (guid()) {
            ...
            if ($html->varExists('geo_position_data')) {
                ...
                $geoPositionData = User::getGeoPosition($geoInfoCity['city_id']);
                $geoPositionData['city'] = $geoPositionData['geo_position_city'];
                $geoPositionData['country'] = $geoPositionData['geo_position_country'];
                $html->setvar('geo_position_data', User::getGeoPositionData($geoPositionData));
            }
        }
