...
class UsersFilter extends UserFields {
    ...
    public static $visitorSearchFilter = null;
    ...
    public static function location()
    {
        ...
        if($guid) {
            ...
            if(get_param('people_nearby')) {
                $location['country'] = $geoInfo['country_id'];
                $location['state'] = $geoInfo['state_id'];
                $location['city'] = $geoInfo['city_id'];
            } else {
                $location['country'] = get_param('country', $geoInfo['country_id']);
                $location['state'] = get_param('state', $geoInfo['state_id']);
                $location['city'] = get_param('city', $geoInfo['city_id']);
            }
        }
    ...
    public function parseBlock(&$html)
    {
       ...
       // if($uid) {
        if ($uid) {
            ...
            $dataLokingFor = self::getVisitorFilter();
        ...
        if ($filters) {
                ...
                foreach($filtersInfo as $filterInfoKey => $filterInfoValue) {
                    ...
                    if($filterInfoKey == 'status' || $filterInfoKey == 'interest') {
                        continue;
                    }
                    ...
                    $html->parse('extended_search_filter_exists');
                }
        ...
        $searchByRadiusAllCountry = ($radius > intval(Common::getOption('max_search_distance')) && isset($filtersInfo['city']) && $filtersInfo['city']['value']);
        ...
        if ($peopleNearby) {
            if($uid) {
                $locationCityTitle = guser('geo_position_city');
            } else {
                $dataLokingFor['city'] = $location['city_title'];
                if(isset($location['country_title'])) {
                    $dataLokingFor['country'] = $location['country_title'];
                }
            }
    ...
    public static function getVisitorFilter()
    {
        User::getOrientationInfo(0);

        UsersFilter::loadVisitorFilter();

        $filterBasicFields = array(
            'p_age_from' => intval(Common::getOption('users_age')),
            'p_age_to' => intval(Common::getOption('users_age_max')),
            'i_am_here_to' => intval(DB::result('SELECT MIN(id) FROM `const_i_am_here_to`')),
        );

        $filterBasicFieldsFromUserFilter = array(
            'country' => 0,
            'state' => 0,
            'city' => 0,
            'people_nearby' => 0,
        ...
        foreach($filterBasicFields as $field => $defaultValue) {
            $visitorFilter[$field] = self::getVisitorFilterValue($field, get_param($field, $defaultValue));
        }

        foreach($filterBasicFieldsFromUserFilter as $field => $defaultValue) {
            $visitorFilter[$field] = self::getVisitorFilterValue($field, get_param($field, $defaultValue), true);
        }

        $visitorFilter['p_orientation'] = self::getVisitorFilterValue('p_orientation', get_checks_param('p_orientation', PHP_INT_MAX));
        if(is_array($visitorFilter['p_orientation'])) {
            $visitorFilter['p_orientation'] = get_checks_param('p_orientation');
        }

        return $visitorFilter;
    }

    public static function saveVisitorFilter($userSearchFilters)
    {
        self::$visitorSearchFilter['user_search_filters'] = $userSearchFilters;
...
//            'with_photo',
        ...
        foreach($params as $param) {
            $value = get_param($param, null);
            if($value !== null) {
                self::$visitorSearchFilter[$param] = $value;
            } else {
                if(isset(self::$visitorSearchFilter[$param])) {
                    unset(self::$visitorSearchFilter[$param]);
                }
            }
        }

        set_session('visitor_search_filter', self::$visitorSearchFilter);
    }

    public static function loadVisitorFilter($filterJson = '')
    {
        $filter = $filterJson ? json_decode($filterJson, true) : array();
        $params = array(
            'p_age_from' => false,
            'p_age_to' => false,
            'p_orientation' => false,
        ...
        foreach($params as $param => $userFilter) {
            $value = self::getVisitorFilterValue($param, null, $userFilter);
            if($value !== null) {
                $_GET[$param] = $value;
                $filter[$param] = array('field' => $param, 'value' => $value);
            } else {
    ...
    public static function getVisitorFilterValue($option, $defaultValue = 0, $isUserSearchFilterValue = false)
    {
        if(self::$visitorSearchFilter === null) {
            self::$visitorSearchFilter = get_session('visitor_search_filter', array());
            if(isset(self::$visitorSearchFilter['user_search_filters'])) {
                self::$visitorSearchFilter['user_search_filters'] = json_decode(self::$visitorSearchFilter['user_search_filters'], true);
            }
        }

        if($isUserSearchFilterValue) {
            $value = isset(self::$visitorSearchFilter['user_search_filters'][$option]) ? self::$visitorSearchFilter['user_search_filters'][$option]['value'] : $defaultValue;
        } else {
            $value = isset(self::$visitorSearchFilter[$option]) ? self::$visitorSearchFilter[$option] : $defaultValue;
        }

        return $value;
    }

    public static function getVisitorFilterValueInArrayFormat($option)
    {
        $result = array();
        $value = self::getVisitorFilterValue($option, null);
        if($value !== null) {
            $result[] = $value;
        }
