...
Class Common {
   ...
   static function getLoginPage()
    ...
    static function setSiteOptions()
    {
        ...
        // M4 (2026-04-23): pre-launch gate. Runs once per request, right after
        // maintenance-mode handling so both modes can coexist.
        $prelaunchFile = dirname(__FILE__) . '/prelaunch.class.php';
        if (is_readable($prelaunchFile)) {
            include_once($prelaunchFile);
            if (class_exists('Prelaunch')) {
                Prelaunch::apply();
            }
        }

        // 2026-05-09: email-verification gate. Logged-in users with a still-
        // unconfirmed mail address (user.active_code != '') are bounced to
        // /email_not_confirmed.php so fake-email registrations can't get
        // past the email link. Runs after Prelaunch and before Onboarding so
        // unconfirmed users never reach the onboarding cards.
        $emailVerifyFile = dirname(__FILE__) . '/email_verification.class.php';
        if (is_readable($emailVerifyFile)) {
            include_once($emailVerifyFile);
            if (class_exists('EmailVerification')) {
                EmailVerification::apply();
            }
        }

        // M3 (2026-04-25): post-registration onboarding gate. New accounts with
        // user.onboarding_done = 0 get bounced to /onboarding.php before any
        // forced-photo / forced-about / profile-completion redirect can fire.
        $onboardingFile = dirname(__FILE__) . '/onboarding.class.php';
        if (is_readable($onboardingFile)) {
            include_once($onboardingFile);
            if (class_exists('Onboarding')) {
                Onboarding::apply();
            }
        }

        // M2 (2026-04-25): server-side visibility filter. Removes user_var
        // entries the current viewer isn't allowed to see for the current
        // request. Encoded core renderers iterate over $g['user_var'] and
        // automatically inherit the filtered view.
        $visFile = dirname(__FILE__) . '/visibility_filter.class.php';
        if (is_readable($visFile)) {
            include_once($visFile);
            if (class_exists('VisibilityFilter')) {
                VisibilityFilter::apply();
            }
        }

        // 2026-05-01: per-page <title> + meta description override. Sets
        // $g['main']['title'] / ['description'] for known pages so each
        // surface gets a distinct, SEO-meaningful title instead of the
        // sitewide default. parseSeoSite() short-circuits to these when
        // both are set.
        $titlesFile = dirname(__FILE__) . '/page_titles.class.php';
        if (is_readable($titlesFile)) {
            include_once($titlesFile);
            if (class_exists('PageTitles')) {
                PageTitles::apply();
            }
        }
    ...
    static function iosAppVersion()
    {
        $version = 0;

        if(isset($_SERVER['HTTP_USER_AGENT'])) {
            $pattern = '/(IOSWebview\-(?<app>[\d\.]*))/';
            if(preg_match($pattern, $_SERVER['HTTP_USER_AGENT'], $matches)) {
                if(isset($matches['app']) && $matches['app'] != '') {
                    $version = $matches['app'];
                }
...
//                }
    ...
    static function devCustomJs(&$html)
    {
        ...
        if (FOOTER_CUSTOM_JS && $html->varExists('footer_custom_js')) {
            ...
            $analyticsCode = '';
            if ($g['main']['site_part'] != 'administration' || IS_DEMO) {
               $analyticsCode = '</script>' . Common::getOption('analytics_code', 'main') . '<script>';
            }

            if(IS_DEMO) {
                $analyticsCode = Demo::addPresaleChatCode($analyticsCode, $tmplLoaded);
            }
...
JS;
    ...
    static function sendMail($admin = false, $typeAutoMail = 'mail_message')
    {
        ...
        if ($userTo && $userFrom && $subject && $text)
        {
            ...
            if ($id && !$block && !$to_myself && !$empty_text)
            {
                ...
                if ($typeAutoMail != 'welcoming_message' && Common::isEnabledAutoMail($typeAutoMail)) {
        ...
        if (in_array($v['type'], $typeFields) && $v['status'] == 'active') {
            if ($v['type'] == 'from'){
                ...
                if (!empty($params)){
                    ...
                    $from_add .= " LEFT JOIN users_checkbox AS " . $nameTable . " ON " . $nameTable . ".user_id = {$tableUser}.user_id AND " . $nameTable . ".field = " . to_sql($v['id'], 'Number') . " AND "  . $nameTable . ".value IN (" . to_sql_array($params) . ")";
        ...
        if(Common::isOptionActive('seo_friendly_urls')) {
            ...
            if ($isCustomUrls) {
                $customUrls = array(
                    ...
                    'user_photos_private_list'  => 'photos_private',
                    'user_vids_list'    => 'vids',
            ...
            $urls = array(
                ...
                'user_photos_private_list'    => 'photos_list.php?uid=' . $uid . '&private=1',
    ...
    static public function saveImageFromString($imageString, $imageType)
    {
        $imageFileBasename = false;
        $image = Image::getImageFromString($imageString);
        if($image) {
            $imageFileBasename = 'tmp_drawer_' . get_param('type') . '_' . microtime(true) . '_' . rand(0, 100000);
            $imageFile = Gallery::getTempFileUploadImageIm($imageFileBasename);

            $im = new Image();
            $im->loadImageFromString($image);
            if($imageType === 'png') {
                $im->saveImage($imageFile, -1, 'png');
            } else {
                $im->saveImage($imageFile, self::getOption('quality', 'image'), 'jpg');
            }
        }

        return $imageFileBasename;
    }

    static public function parseDrawerBlock(&$html, $block)
    {
        if(self::isOptionActive('drawer_enabled')) {
            $html->parse($block, false);
        }
    }

    static public function validateExistsTmpFilePath($tmpFilePath)
    {
        $isValid = true;
        $realFilePath = realpath($tmpFilePath);
        $dirFilesPath = realpath(Common::getOption('dir_files', 'path') . 'temp' . DIRECTORY_SEPARATOR);
        if(strpos($realFilePath, $dirFilesPath) === false) {
            $isValid = false;
        }
