...
class CIm extends CHtmlBlock
{
	...
	static function addMessage(&$html, $userTo = null, $msg = null, $parseMsg = true)
    {
        ...
        $imSendImage = Common::isOptionActiveTemplate('im_send_image');
 		...
 		if ($g_user['user_id'] && $userTo
                && ($msg != '' || $audioMessageId || $imageUpload || $imageDrawer)) {
			$optionTmplName = Common::getOption('name', 'template_options');
            $blockMsg = 'message';
            $isFreeSite = Common::isOptionActive('free_site');
            ...
            if ($notMsgToDb) {
                ...
                if($imageDrawer) {
                    $msg = Gallery::uploadImageFromDrawerForText($msg, false);
                    if(strpos($msg, '{img_upload:') !== false) {
                        $system = 1;
                        $systemType = 2;
                    }
                }
		...
		if ($userInfo) {
            ...
            if (isset($userInfo['last_msg'])) {
                ...
                if ($userInfo['last_msg']) {
                    ...
                    $sql = 'SELECT * FROM `' . self::getTable(true) . '`
                             WHERE
                               ((`from_user_deleted` = 0 AND `from_user` = ' . to_sql($g_user['user_id']) . ' AND `to_user` = ' . to_sql($toUser) . ')
                                OR (`to_user_deleted` = 0 AND `from_user` = ' . to_sql($toUser) . ' AND `to_user` = ' . to_sql($g_user['user_id']) . ')) '
                ...
                if (($userInfo['last_msg_from_user'] == guid() || (isset($lastMsgInfo) && $lastMsgInfo['msg'] === 'you_granted_access')) && $lastMsg) {
    ...
    static function grabsRequestNotif($msg, $system, $audioMessageId = 0)
    {
        ...
        foreach ($types as $type) {
            if (stristr($msg, $type) !== false) {
                if ($type == '{gift:') {
                ...
                } else if ($type == 'you_granted_access') {
                    ...
                    if ($typeIm == 'edge') {
                        // double parser for img_upload in image description
                        for($i = 0; $i < 2; $i++) {
            ...
            foreach ($types as $type) {
                ...
                if($parse) {
                    if ($type == 'private_photo_request') {
                        ...
                        $attrLink = array();
                        $vars = array('url' => (Common::getOptionTemplate('im_type') === 'edge' ? Common::pageUrl('user_photos_private_list', $msgUserId) : 'search_results.php?display=profile&uid=' . $msgUserId . '&show=gallery'));
                        ...
                        if ($image) {
                            ...
                            $craftedMsg = str_replace('{img_upload:' . $img[0] . '}', '', $msg) . '<div class="mod_im_msg_image">'
	...
	function parseBlock(&$html)
	{
        ...
        if ($guid && in_array($cmd, $allowedCmd)) {
			...
			if ($notUploadingMsg && !$showIm) {
                ...
                Common::parseDrawerBlock($html, 'im_drawer');
	...
	static function uploadImageChangeEdit()
	{
		...
		if (!$fileUrl || !Common::validateExistsTmpFilePath($fileUrl)) {
			return false;
		}
