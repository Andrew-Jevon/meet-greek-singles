...
class CProfilePhoto extends CHtmlBlock
{
    ...
    static $wherePrivateActive = false;

    static $excludedPrivatePhotoIds = array();
    ...
    static function photoUpload($dir = '', $inputFile = '', $isCity = false, $uploadDefault = false)
    {
        global $g, $g_user;
		...
		if (!$isCity && Common::getTmplName() == 'edge') {
			$result['src_bm'] = User::getPhotoFile($photo, 'bm', '');
		}
    ...
    static function photoRotate()
    {
        ...
        if ($isRotatedSrcFileSaved && $im->image) {
            ...
            $sFile_ = self::createBasePhotoFilePath($uid, $pid, $photo['hash']);
    ...
    static function updateFromString($param = 'image', $uid = null)
    {
        global $g;
	...
	static function createFileOrigImage($fileNameSrc)
    {
	...
	static function restoreImage($uid = null)
    {
        global $g;
		...
		if ($im->image) {
			$sFile_ = CProfilePhoto::createBasePhotoFilePath($uid, $pid, $photo['hash']);
    ...
    static function getPhotoListMobile($uid, $order = '`photo_id` DESC', $whereSql = '', $limit = '', $groupId = 0, $private = false)
    {
        global $g_user;

        $photosInfo = self::preparePhotoList($uid, $order, $whereSql, $limit, false, false, false, $groupId, true, $private);
    ...
    static function preparePhotoList($uid, $order = '`private` DESC, `default` ASC, `photo_id` DESC', $whereSql = '', $limit = '', $isVisible = false, $isCity = false, $onlyPublic = false, $groupId = 0, $showAllMyPhoto = false, $private = false)
    {
        global $g, $g_user;
        ...
        if(IS_DEMO) {
            $whereSql .= Demo::appHideOldPrivatePhotoWhere();
        }
        ...
        if ($isEdge) {
            ...
            $where = self::getWherePhotosList('PH.', $onlyPublic, $uid, $groupId, $showAllMyPhoto, $private);
            if ($whereSql) {
                $where .= $whereSql;
            }
        ...
        if (!empty($profilePhoto)) {
            ...
            $isPrivatePhotoAvailable = User::hasPrivatePhotoAccess($uid, $g_user['user_id']);
            $isPrivatePhotoRequestExists = User::isFriendRequestExists($uid, $g_user['user_id']);

            self::$wherePrivateActive = true;
            $countPhotoPublic = intval(self::getTotalPhotos($uid, false, 0, false));
            $countPhotoPrivate = intval(self::getTotalPhotos($uid, false, 0, true));
            self::$wherePrivateActive = false;

            foreach ($profilePhoto as $item) {
                if($item['private'] === 'Y' && !$isPrivatePhotoAvailable) {
                    $item['hash'] = '';
                    $item['description'] = '';
                }
                $item['isPrivatePhotoRequestExists'] = $isPrivatePhotoRequestExists;
                $item['countPhotoPublic'] = $countPhotoPublic;
                $item['countPhotoPrivate'] = $countPhotoPrivate;
                $uid = $item['user_id'];
                if ($isEdge) {
                ...
                $pid = $item['photo_id'];
                $photoInfo = array('description'=> $item['description'], 'version' => $item['version'], 'visible' => $item['visible'], 'gif' => $item['gif'], 'hash' => $item['hash'], 'isPrivatePhotoRequestExists' => $isPrivatePhotoRequestExists, 'countPhotoPublic' => $countPhotoPublic, 'countPhotoPrivate' => $countPhotoPrivate);
                if ($item['private'] == 'Y') {
                    $photoInfo['private'] = 'Y';
                ...
                self::$allPhotoInfo[$pid]['isPrivatePhotoRequestExists'] = $item['isPrivatePhotoRequestExists'];
                self::$allPhotoInfo[$pid]['countPhotoPublic'] = $item['countPhotoPublic'];
                self::$allPhotoInfo[$pid]['countPhotoPrivate'] = $item['countPhotoPrivate'];
                ...
                self::$allPhotoInfo[$pid]['gender'] = $gender;
            }
    ...
    static function prepareVideoList($uid, $order = '`private` DESC, `id` DESC')
    {
        ...
        if (!empty($profileVideo)) {
            ...
            foreach ($profileVideo as $item) {
                ...
                $pidV='v_'.$item['id'];
                if ($item['private'] == 1) {
                    self::$privateVideo[$pid] = array('id' => $item['id'],
													  'user_id' => $uid,
													  'live_id' => intval($item['live_id']),
													  'private' => 'Y',
													  'description' => $item['subject']);
                } else {
                    self::$publicVideo[$pid] =  array('id' => $item['id'],
													  'user_id' => $uid,
													  'live_id' => intval($item['live_id']),
													  'private' => 'N',
													  'description' => $item['subject']);
...
//            User::setNoPhotoPprivateInOffset(false);
    ...
    static function setPhotoPrivate($id, $isAdmin = false)
    {
        ...
        if ($guid && $id) {
            ...
            $sql = 'SELECT `private`, `visible`, `nudity`, `default`
                      FROM `photo`
            ...
            if ($private == 'N') {
                ...
                if ($photo['visible'] == 'Y') {
                    $pid = User::getPhotoDefault($guid, 'r', true, guser('gender'));
                    if($pid == $id) {
                        $responseData = 'update_set_default';
                    }
                }
            }
        }
        ...
        if (get_param_int('get_data_edge')) {
            $publicPhotoCount = self::getTotalPhotos($guid, true, 0);
            $data = array(
                'result' => $responseData,
                'count_public' => $publicPhotoCount,
                'count_title' => lSetVars('edge_column_photos_title', array('count' => $publicPhotoCount)),
            );

            if($responseData === 'update_set_default') {
                self::preparePhotoList($guid);
                $data['photo_public_data'] = array(self::$allPhotoInfo[$pid]);
                $data['photo_default'] = User::getPhotoDefault($guid, 'r', true);
                $data['isImageEditorEnabled'] = Common::isImageEditorEnabled();
            }
    ...
    static function deletePhoto($id, $getPhotoDefaultId = false, $uid = null, $admin = false)
    {
        ...
        if(strpos($id,'v_') === 0){
            ...
            if (get_param_int('get_data_edge')) {
                ...
                if(!$groupId) {
                    $numberPhotoPrivateCount = self::getTotalPhotos($guid, false, $groupId, true);
                } else {
                    $numberPhotoPrivateCount = 0;
                }
                ...
                $responseData = array('photos_info' => array(),
                                      ...
                                      'count_private_title' => lSetVars('edge_column_photos_private_title', array('count' => $numberPhotoPrivateCount)),
                                      'count' => $numberPhoto,
                                      'count_private' => $numberPhotoPrivateCount,
                                      'group_id' => $groupId);
            } elseif (!$admin) {
    ...
    static function publishPhotos($photos = null, $type = null, $groupId = null, $uploadDefault = false)
    {
        global $g;
        global $g_user;
    ...
    static function setPhotoDefault($pid = 0)
    {
        $responseData = false;
    ...
    static function addComment($isNotifComments = true)
    {
        ...
        $imageDrawer = get_param('image_drawer');
        if ($photoId && ($msg != '' || $audioMessageId || $imageUpload || $imageDrawer)) {
    ...
    static function updateLikeComment($type = null, $id = null)
    {
        ...
        if ($type == 'photo') {
            $cacheWallIdKeys = 'CProfilePhoto.photo.wall_id_for_id' . $id;
            $wallId = Cache::get($cacheWallIdKeys);
            if($wallId === null) {
                $wallId = DB::result('SELECT `wall_id` FROM `photo` WHERE `photo_id` = ' . to_sql($id));
                Cache::add($cacheWallIdKeys, $wallId);
            }
    ...
    static function parseRepliesComments(&$html, $cid, $numberReplies, $alwaysView = false, $commentsLikes = null, $type = 'photo', $isWall = false, $showEventReplyCommentId = false)
    {
        ...
        if ($isWall) {
            ...
            if ($cmd == 'update_im' && Wall::$commentCustomClass != 'comment_attach'
                    && Wall::$commentCustomClass != 'comment_attach_reply_add') {
                $where = ' AND c.id > ' . to_sql($lastId);
                $limit = 0;
        ...
        $replies = DB::all($sql);
        if ($cid == 3151) {
           // var_dump_pre($_GET['last_id']);
            //var_dump_pre(get_param('last_id'));
            //echo $sql;
            //exit();
        }
    ...
    static function parseComment(&$html, $comment, $block = 'comment', $type = 'photo', $isWall = false)
    {
        ...
        if (!empty($comment)) {
			...
			if ($block == 'comment') {
                ...
                Common::parseDrawerBlock($html, 'comment_reply_drawer');
			}
    ...
    function parseBlock(&$html)
	{
        ...
        if ($cmd == 'photo_comment_add'  || $cmd == 'set_rate_photo') {
            ...
            if (get_param_int('get_data_edge')) {
                ...
                $isDelete = ($pid != $curPhoto && !isset(self::$privatePhoto[$pid]));
                $photoDefault = 0;
                if ($isDelete) {
                ...
                $isCurPhotoPrivate = false;

                if($curPhoto) {
                    $isCurPhotoPrivate = ($list[$curPhoto]['private'] === 'Y');
                    self::$wherePrivateActive = true;
                    $numberPhotoProfile = intval(self::getTotalPhotos($uid, false, 0, $isCurPhotoPrivate));
                    self::$wherePrivateActive = false;
                }

                $html->setvar('count', $numberPhoto);
                $html->setvar('count_profile', $numberPhotoProfile);
                $html->setvar('count_profile_title', lSetVars(($isCurPhotoPrivate ? 'edge_column_photos_private_title' : 'edge_column_photos_title'), array('count' => $numberPhotoProfile)));
...
//            var_dump(self::$allVideoInfo);exit;
    ...
    /* All photos site */
    static function getTotalPhotos($uid = 0, $onlyPublic = false, $groupId = 0, $private = false)
    {
        ...
        if ($whereTags) {
            $where = self::getWherePhotosList('PH.', $onlyPublic, $uid, $groupId, $private);
            ...
            $where = self::getWherePhotosList('', $onlyPublic, $uid, $groupId, false, $private);

            $count = DB::count('photo', $where);
            return $count;
        }
    }

    static function getWherePhotosList($table = '', $onlyPublic = false, $uid = 0, $groupId = 0, $showAllMyPhoto = false, $private = false)
    {
        $guid = guid();
        $where = " {$table}visible != 'P' ";
        ...
        if($optionTemplateName == 'edge') {
            $cmd = get_param('cmd');
            // show only public photo in gallery
            if(get_param_int('uid') === 0 && get_param_int('get_data_edge') && $cmd === 'get_photo_comment') {
                $where .= " AND {$table}private = 'N'";
            } elseif(self::$wherePrivateActive || $cmd === 'delete_photo' || (!get_param_int('get_data_edge') && $cmd !== 'get_photo_comment')) {
                $where .= " AND {$table}private = " . to_sql($private ? 'Y' : 'N') . " ";
            }
        }
    ...
    static function getOrderList($typeOrder = '')
    {
        ...
        if(Common::tmplName() === 'edge') {
            $orderBy = 'PH.`private` DESC,' . $orderBy;
        }
    ...
    static function getPhotosList($typeOrder, $onlyPublic = true, $limit = '0, 4', $uid = null, $groupId = 0, $showAllMyPhoto = false, $private = false)
    {
        global $g;
        ...
        $photos = self::preparePhotoList($uid, $orderBy, '', $limit, false, false, $onlyPublic, $groupId, $showAllMyPhoto, $private);
	...
	static function saveFaceDetectData($pid = null, $data = null, $checkPhotoUser = true)
    {
	...
	static function updateFaceDetectFriend($pid, $boxId, $uid)
	{
		$guid = guid();
	...
	static function setFaceDetectFriend()
    {
		$pid = get_param_int('pid');
		...
		$response = array('data' => $faceDetectData,
						  'title' => self::getFaceDetectTitle($faceDetectData));

		return $response;
	}

	static function removeRelationUserFaceDetect($uid, $photoUid = null)
	{
	...
	static function removeRelationUnfriendFaceDetect($uid, $fid)
	{
	...
	static function clearFaceDetectFriend()
    {
		$pid = get_param_int('pid');
		$boxId = get_param_int('box_id');
		$guid = guid();
	...
	static function clearFaceDetectAllFriend()
    {
		global $sitePart;
	...
	static function getFaceDetectTitle($faceDetectData)
	{
	...
	static function prepareFaceDetectFriend($faceDetectData)
	{
    ...
    static function parseCommentDelete(&$html, $comment, $blockDelete = 'comment_delete')
    {
	...
	static function parseImageEditor(&$html, $block = 'image_editor')
    {
