...
class LiveStreaming extends CommentsBase
{
    ...
    static function createLive()
    {
        ...
        if ($uid) {
            ...
            DB::update('vids_video', array('live_id' => $liveId), '`id` = ' . to_sql($videoId));
                        ...
                        if ($fileLiveSave){
                            ...
                            foreach ($filesImage as $key => $ext) {
                                ...
                                if (file_exists($fileExists)) {
                                    rename($fileExists, $g['path']['dir_files'] . 'video/' . $id . $ext);
                                }
    ...
    static function parseBlockComments(&$html, $id)
    {
        ...
        if ($comments_enabled && $id) {
            ...
            Common::parseDrawerBlock($html, 'ls_comment_top_drawer');
            Common::parseDrawerBlock($html, 'ls_comment_bottom_drawer');
    ...
    static function addComment()
    {
        ...
        if ($videoId && ($msg != '' || $audioMessageId || $imageUpload || $imageDrawer)) {
