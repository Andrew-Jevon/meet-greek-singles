...
class CJoinForm extends CHtmlBlock {
    ...
    function init()
    {
        ...
        if ($cmd == 'register') {
            ...
            // 2026-05-01: home_city is the user-typed free-text city, written to
            // userinfo.home_city via the user_var registered by
            // _mgs_homecity_install.php. Captured here, sent forward through
            // the session, persisted after User::add() returns the uid (mobile
            // path below + desktop path in cjoinfinal.class.php).
            $homeCityRaw = trim((string) get_param('home_city', ''));
            if ($homeCityRaw !== '') {
                $homeCityRaw = mb_substr($homeCityRaw, 0, 60, 'UTF-8');
            }
            ...
            /* URBAN */
            if ($this->ajax) {
                /* Patched 2026-04-30 per Irene: city is now an optional manual
                   text input. Server-side validateLocation() rejects city=0
                   (which is what intval() of text returns) regardless of the
                   4th flag, so we skip the location validator entirely.
                   Country/state are still required client-side (the submit
                   button stays disabled until they're picked). */
                $validate = '';
            ...
            if ($this->message == "" && $this->responseData == '') {
                ...
                set_session("j_home_city", $homeCityRaw);
                if ($isCustomRegister) {
                ...
                if (Common::isMobile() || ($this->ajax && !$isCustomRegister)) {
                    ...
                    if (!$uid) {
                        ...
                        $g_user['user_id'] = $uid;
                        // 2026-05-01: persist user-typed city to userinfo.home_city.
                        // Field registered as user_var; column added by
                        // _mgs_homecity_install.php. Empty string is a valid value.
                        $hc = (string) get_session('j_home_city', '');
                        DB::execute('UPDATE userinfo SET home_city = ' . to_sql($hc, 'Text')
