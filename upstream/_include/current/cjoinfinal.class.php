...
class CJoinFinal extends UserFields//CHtmlBlock
{
        ...
        if (in_array($cmd, $cmds)) {
            ...
            if ($this->message == '') {
                ...
                if (!$uid) {
                    ...
                    $g_user['user_id'] = $uid;
                    self::persistHomeCity($uid);
                    if ($cmd == 'photo') {
                        uploadphoto($g_user['user_id'], get_param('photo_name', ''), get_param('description', ''), ($g['options']['photo_approval'] == 'Y' ? 0 : 1));
                    }
                    //Common::toHomePage();
                    $this->goToHomePage();
                }
    ...
    /** 2026-05-01: persist user-typed city captured by cjoinform into the
     *  user_var-backed userinfo.home_city column, immediately after the new
     *  user row exists. Empty-string is a valid (default) value. */
    static function persistHomeCity($uid)
    {
