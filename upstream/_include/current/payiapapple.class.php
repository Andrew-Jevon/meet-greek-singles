...
-----END CERTIFICATE-----';
    ...
    private static function realTimeNotification()
    {
        ...
        } elseif(isset($data['signedPayload'])) {
            $payload = self::decodeAppStoreServerNotificationV2($data['signedPayload']);
            $notificationType = $payload['notification_type'];
            $transaction = $payload['transaction'];
            if(isset($transaction['expiresDate'])) {
                $transaction['expires_date_ms'] = floatval($transaction['expiresDate']);
            }
            $packageName = $transaction['bundleId'];
            $productId = $transaction['productId'];
            $originalTransactionId = $transaction['originalTransactionId'];
        }

        if(isset($notificationType) && isset($transaction['expires_date_ms'])) {
            self::processRealTimeNotification($notificationType, $transaction, $packageName, $productId, $originalTransactionId);
        } else {
    ...
    private static function processRealTimeNotification($notificationType, $transaction, $packageName, $productId, $originalTransactionId)
    {
        ...
        if($payment) {
            ...
            if($notificationType == 'REFUND' || $notificationType == 'CANCEL') {
                $time = time();
    ...
    public static function decodeAppStoreServerNotificationV2($signedPayload)
    {
        $data = array();

        $signedPayloadParts = explode('.', $signedPayload);
        if (count($signedPayloadParts) !== 3) {
            die('Invalid signedPayload');
        }

        $header = json_decode(base64_decode($signedPayloadParts[0]), true);
        if($header && isset($header['alg']) && isset($header['x5c'])) {
            $algorithm = $header['alg'];

            $certificate = self::prepareCertificateText($header['x5c'][0]);
            $certificateintermediate = self::prepareCertificateText($header['x5c'][1]);
            $certificateRoot = self::prepareCertificateText($header['x5c'][2]);

            if (self::openssl_x509_verify($certificateintermediate, $certificateRoot) !== 1){
                echo 'Intermediate and Root certificate do not match';
                exit;
            }

            if (self::openssl_x509_verify($certificateRoot, self::APPLE_ROOT_PEM) === 1){
                $publicKey = self::getPublicKeyFromCertificate($certificate);
                $payload = json_decode(base64_decode($signedPayloadParts[1]));
                //var_dump_pre($payload);
                $data['notification_type'] = $payload->notificationType;
                $data['transaction'] = (array)JWT::decode($payload->data->signedTransactionInfo, $publicKey, array($algorithm));
            } else {
    ...
    static public function prepareCertificateText($certificate)
    {
        $certificateText = "-----BEGIN CERTIFICATE-----\n$certificate\n-----END CERTIFICATE-----";
        return $certificateText;
    }

    static public function getPublicKeyFromCertificate($certificate)
    {
    ...
    static public function openssl_x509_verify($certificate, $publicKey)
    {
        $isVerified = 0;

        $x509 = new X509();
        $x509->loadX509($certificate);
        $x509->loadCA($publicKey);

        if ($x509->validateSignature()) {
            if ($x509->validateDate()) {
                $isVerified = 1;
            } else {
                //die('Certificate date is incorrect');
            }
        } else {
            //die('validateSignature incorrect');
        }
