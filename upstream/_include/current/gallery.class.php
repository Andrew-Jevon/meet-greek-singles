...
class Gallery
{
    ...
    static function imageDelete($id, $uid, $deleteEmptyAlbum = true)
    {
        ...
        if($row = DB::fetch_row(1)) {
            ...
            $imageIdInDescription = grabs($row['desc'], '{img_upload:', '}');
            if (isset($imageIdInDescription[0])) {
                self::imageDelete($imageIdInDescription[0], $uid, $deleteEmptyAlbum);
            }
        }
    }
    ...
    static function uploadImageFromDrawerForText($text, $checkReplies = true, $checkRepliesClear = true, $paramName = 'image_drawer')
    {
        $imageDrawer = get_param($paramName);
        if($imageDrawer) {
            $imageFileBasename = Common::saveImageFromString($imageDrawer, 'png');
            if($imageFileBasename) {
                if($checkReplies) {
                    $replies = Wall::checkRepliesUserComment($text, $checkRepliesClear);
                    $text = $replies['comment'];
                }
                $imgId = Gallery::uploadIm($text, $imageFileBasename);
                if ($imgId) {
                    $text = ($checkReplies ? ($replies['tag'] . ' ') : '') . '{img_upload:' . $imgId . '}';
                }
            }
        }
