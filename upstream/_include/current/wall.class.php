...
class Wall {
    ...
    static function add($section, $item = 0, $uid = false, $params = '', $unique = false, $hideFromUser = 0, $access = 'public', $parent = 0, $paramsSection = '', $groupId = null, $vidsNoLoad = 0)
    {
	...
	static function parseCommentImg(&$comment, $wallPost = false)
	{
		global $g;
		global $sitePart;
        ...
        for($i = 0; $i < 2; $i++) {
            ...
            if (isset($img[0])) {
                ...
                $imageUrlBig = $urlFiles . 'gallery/images/' . $image['user_id'] . '/' . $image['folder'] . '/' . $image['filename'] . '?id=' . $image['id'];
                ...
                $tagHtml = $userUrl .
                            '<div class="image_comment">' .
                                '<a data-id="' . $tagId . '" class="' . ($wallPost ? 'timeline_photo' : 'timeline_photo_comment') . ' lightbox ' . $tagId . '" href="' . $imageUrlBig . '">' .
                ...
                if (self::checkTypeWall('edge') && $sitePart != 'administration') {
                    ...
                    if($wallPost) {
                        $tagHtml = str_replace('image_comment', 'wall_image_post', $tagHtml);
                    }
                }

                $comment['comment'] = str_replace('{img_upload:' . $img[0] . '}', '', $comment['comment']) . $tagHtml;
                if($replies['tag']) {
                    $comment['comment'] = str_replace($replies['tag'], '', $comment['comment']);
                }
    ...
    static function parseComments(&$html, $id, $index = 2, $start = 0, $limit = 3, $cid = 0, $where = '', $row = false)
    {
        ...
        $mediaSection = '';
        $commentsLikes = array();
        if ($isEdgeWall && ($row === false || $row['section_real'] != 'share')) {
			$where .= $whereParent;
            if ($row === false) {
                $row = DB::row('SELECT * FROM `wall` WHERE `id` = ' . to_sql($id));
            }
        ...
        if ($guid && $html->blockExists($blockFeedBottomFrm)) {
        ...
        Common::parseDrawerBlock($html, 'wall_comment_drawer_top');
        Common::parseDrawerBlock($html, 'wall_comment_drawer_bottom');
    ...
    static function addInfoComment(&$html, &$vars, &$row)
    {
        ...
        if ($optionNameTemplate == 'edge') {
            ...
            $commentTmp = array('comment' => $comment);
            if(self::parseCommentImg($commentTmp, true)) {
                $comment = $commentTmp['comment'];
            }
        } else {
...
WHERE forum_id = ' . $comment['forum_id'];
    ...
    static function addInfoPics(&$html, &$vars, $row)
    {
        ...
        $parseFirstImage = false;
        if($vars['photo_count'] == 1) {
            $imgIdInDescription = grabs($rows[0]['desc'], '{img_upload:', '}');
            if($imgIdInDescription) {
                $sql = 'SELECT * FROM gallery_images
                         WHERE id = ' . to_sql($imgIdInDescription[0]);
                $imageInfo = DB::row($sql);
                if($imageInfo) {
                    $rows[] = $imageInfo;
                    $parseFirstImage = true;
                }
            }
        }
    ...
    static function parseItems(&$html, $id = false, $oneOnly = false, $newOnly = false, $isNumberPostsUser = false, $groupId = null, $admin = false)
    {
        ...
        if ($optionTemplateSet == 'urban') {
            ...
            if (self::$typeWall == 'edge') {
                ...
                if (is_array($sectionsOnly) && in_array('interests', $sectionsOnly)) {
...
//            die();
    ...
    {
        $comment = Gallery::uploadImageFromDrawerForText($comment, true);
    ...
    static function addComment($id, $uid = false)
    {
        ...
        if($comment != '' || $audioMessageId || $imageUpload || $imageDrawer) {
