...
function send_mail($to_mail, $from_mail, $subject, $message, $name = NULL)
{
    ...
    if($name != '') {
        $fromNameEncoded = '"=?utf-8?b?' . base64_encode(trim($name)) . '?=" ';
    }

    $headers = "";
    $headers .= "From: " . $fromNameEncoded . "<" . $from_mail . ">" . "\n";
    ...
    $clientErrorOffSrc = isset($g['client_error_off']) ? $g['client_error_off'] : false;

    if(!isset($sitePart) || ($sitePart !== 'administration')) {
        $g['client_error_off'] = true;
    }

    // Hosting blocks outbound SMTP (587/465/25). Try SMTP when enabled, but
    // fall back to the server's local sendmail path when relay is unreachable.
    if (Smtp::isActive()) {
        if (send_mail_smtp($to_mail, $from_mail, $subject, $message, $name)) {
