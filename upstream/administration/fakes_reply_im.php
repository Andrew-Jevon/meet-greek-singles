...
class FakesReplyIm extends CHtmlList
{
	...
	function onItem(&$html, $row, $i, $last)
	{
        ...
        $html->setvar('user_from_photo', $this->getUserPhoto($row['from_user']));
        $html->setvar('user_to_photo', $this->getUserPhoto($row['to_user']));
        ...
        if ($row['u_from_fake'] == 1) {
            $messageTypeClass = 'message_replied';
            if (!$row['is_new']) {
        ...
        $html->setvar('message_type_class', $messageTypeClass);

	}

    function getUserPhoto($userId)
    {
        $keyPhoto = 'user_photo_im_' . $userId;
        $userPhotoUrl = Cache::get($keyPhoto);
        if ($userPhotoUrl === null) {
            $userPhotoUrl = User::getPhotoDefault($userId, 'r', false);
