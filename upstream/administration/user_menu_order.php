...
class CMobileUserMenu extends CHtmlBlock {
    ...
    function action() {
        ...
        if($cmd == 'update') {
            ...
            $page = get_param('page');
            DB::delete('col_order', '`system` = 0 AND `id` = ' . to_sql($page) . ' OR `parent` = ' . to_sql($page));
    ...
    function parseBlock(&$html) {
        ...
        DB::query("SELECT * FROM `col_order` WHERE `lang` = 'default' AND `section` = '{$section}' {$where} ORDER BY position");
        ...
        while ($row = DB::fetch_row()) {
            ...
            if ($row['system']) {
                $menuTitle = l('user_menu_' . $row['name'] . $prf, $lang);
                $html->clean('delete');
            } else {
                $menuTitle = $row['name'];
            ...
            $html->setvar('name_block', $menuTitle);
