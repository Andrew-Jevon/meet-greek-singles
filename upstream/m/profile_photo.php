...
class CPhoto extends CHtmlBlock
{
        ...
        if ($isAjaxRequest && $cmd != 'insert') {
            ...
            if ($cmd == 'publish_one_photo') {
                ...
                if ($g_user['user_id'] && $pid) {
                    $photo = DB::one('photo', '`photo_id` = ' . to_sql($pid) . ' AND `user_id` = ' . to_sql($g_user['user_id']));
                    if($photo) {
                        $photos = array(
                            array(
                                'id' => $photo['photo_id'],
                                'time_upload' => strtotime($photo['date']),
                                'desc' => $photo['description'],
                            ),
                        );
                        $responsePublish = CProfilePhoto::publishPhotos($photos, $photo['private'] === 'N' ? 'public' : 'private');
                        $responseData = ((isset($responsePublish[$pid]['visible']) && $responsePublish[$pid]['visible'] === 'Y') ? 'Y' : 'N');
	...
	function parseBlock(&$html)
	{
        ...
        $sql = "SELECT * FROM photo WHERE user_id=" . $g_user['user_id'] . "  AND group_id = 0 AND visible != 'P' " . (IS_DEMO ? Demo::appHideOldPrivatePhotoWhere() : '') . " ORDER BY photo_id DESC;";
		DB::query($sql);
