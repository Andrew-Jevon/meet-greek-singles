    ...
    static function parseMenu(&$html)
	{
        ...
        if ($html->blockExists($blockUserMenu)) {
            ...
            if (!Common::isOptionActive('photo_rating_enabled')) {
                $where = " AND C.`name` != 'photo_rating'";
            }
            $limit = '';
            if (self::$type == 'profile_view') {
                $where .= " AND C.`additional` = 1";
                $limit = ' LIMIT 9';
            }
            $sql = "SELECT C.*, C2.name AS name_translated
                      FROM `col_order` AS C
                      LEFT JOIN `col_order` AS C2 ON C.id = C2.parent AND C2.lang = " . to_sql($g['lang_loaded']) . "
                     WHERE C.`section` = '{$table}'
                       AND C.`status` = 'Y'
                       AND C.`lang` = 'default'
                       {$where}
            ...
            if ($typeMenu == 'impact') {
				if (LiveStreaming::isAviableLiveStreaming()) {
					$pageUrl['live'] = Common::pageUrl('live', guid());
					if (!User::accessCheckFeatureSuperPowers('live_streaming')) {
						$pageUrl['live'] = $urlPageUpgrade;
					}
					$pageUrl['live_list'] = Common::pageUrl('live_list');
					$pageUrl['live_list_finished'] = Common::pageUrl('live_list_finished');
				}
                ...
                if (User::isAllowProfileVerification()) {
                    $pageUrl['verify_account'] = '';
                }
            ...
            while ($row = DB::fetch_row(1)) {
                $isSystem = ($row['system'] == 1);
                if(!isset($pageUrl[$row['name']]) && $isSystem){
                    continue;
                }
                if (self::$type == 'profile_view') {
                ...
                $html->setvar("{$blockUserMenu}_id", $row['id']);
                $html->setvar("{$blockUserMenu}_name", $isSystem ? $row['name'] : 'text_page');
                $html->setvar("{$blockUserMenu}_title", $isSystem ? l('user_menu_' . $row['name'] . $prf) : ($row['name_translated'] ? $row['name_translated'] : $row['name']));
                $html->setvar("{$blockUserMenu}_url", $isSystem ? $pageUrl[$row['name']] : Common::pageUrl('page') . '?id=' . $row['id']);
                ...
                if (!in_array($row['name'], array('encounters', 'people_nearby', 'photo_rating'))) {
                    ...
                    } elseif ($row['name'] == 'live_list') {
						$counter = LiveStreaming::getTotalLiveNow();
					} elseif ($row['name'] == 'live_list_finished') {
						$counter = LiveStreaming::getTotalLiveFinished();
					}
                ...
                $blockUserMenuItemTextPage = 'icon_text_page';
                if(!$isSystem) {
                    $html->parse($blockUserMenuItemTextPage, false);
                } else {
                    $html->clean($blockUserMenuItemTextPage);
                }
