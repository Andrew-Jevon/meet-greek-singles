...
function closePopupUpdate(name,update,call){
    if (openPopupList[name]&&openPopupList[name]['el'].is(':visible')){
        update=update||0;
        openPopupList[name]['close']=1;
        var $pp=openPopupList[name]['el'].close(false,call);
        if(!update){
            setTimeout(function(){
...
function checkSupportWebrtc(type){
    ...
    if (is=='ssl') {
        alertCustom(siteLangParts['lMediaChatSsl_'+type], true, ALERT_HTML_ALERT)
        is=false;
    }else if(is===false){
...
function approvePhoto(pid){
    ...
    redirectUrl(urlMain+'moderator.php?do%5B'+pid+'%5D='+cmd+'&section=photo');
}
...
function approvePhotoDelete(pid){
    confirmCustom(
        ...
        function(){
			var params='section=photo&do['+pid+']=del';
			goLink(urlMain+'moderator.php?comment_photo['+pid+']=' + encodeURIComponent(trim($('#comment_photo_'+pid).val())),params);
        }
    )
}
