$(function () {
	...
	function loginResponse(data) {
		...
		if(data.substring(0, 11) == '#js:logged:') {
			// 2026-05-05 per Irene — server returns the username (e.g. "joyce")
			// as the redirect target, which is a vanity URL routed via
			// router.php. During pre-launch, router.php is not in the
			// allowlist, so prelaunch redirects to /?__plfb=1 and the user
			// appears not to be logged in. Use the homepage (always allowed)
			// instead — Chameleon's gates take it from there (sends users
			// with onboarding_done=0 to /onboarding.php, members with done=1
