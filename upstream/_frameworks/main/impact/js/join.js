...
$(function(){
    ...
    $jq('#frm_register_submit_2').click(function(){
        // M4 (2026-04-25): client-side T&C guard. Without this, the loop
        // below treats an unchecked checkbox as if it were checked, because
        // it reads `this.value` (always "1") instead of `this.checked`.
        // Server-side enforcement is also active in join.php as a backstop.
        var $agree = $jq('#agree');
        if ($agree.length && !$agree.prop('checked')) {
            $jq('#agree_error').removeClass('to_hide');
            return false;
        }
        ...
        $jq('#frm_register_submit_2').html(getLoader('css_loader_btn', false, true))
                                     .prop('disabled',true);
        $jq('input, select', '#step-1').each(function(){
            // Checkboxes / radios — only send when actually checked.
            if (this.type === 'checkbox' || this.type === 'radio') {
                if (this.checked) {
                    dataFrm[this.name] = $.trim(this.value);
                } else {
                    delete dataFrm[this.name];
                }
                return;
            }
            dataFrm[this.name]=$.trim(this.value);
        ...
        function(data){
            ...
            if(data!==false){
                ...
                if ($exEmail[0]) {
                    ...
                    showError('captcha','','#frm_card_join');
                }else if ($data.filter('.error_terms')[0]) {
                    // M4: server rejected because T&C wasn't accepted.
                    disabledControl(false, '#frm_card_join');
                    showError('agree');
                    var $agree = $jq('#agree');
                    if ($agree.length && $agree.offset()) {
                        $('html,body').animate({scrollTop: $agree.offset().top - 120}, 300);
                    }
                }
            }
        })
    ...
    $.post('search_results.php?join_search_page=1&ajax=1',
           {with_photo:1,
            ...
            function(data){
                        ...
                        // Pre-launch safeguard (2026-04-25): the AJAX endpoint
                        // (search_results.php) is gate-blocked during pre-launch
                        // and returns the homepage HTML, OR the search returns
                        // members without uploaded photos. Either way, the user
                        // can never click 3 real photos and registration stalls.
                        // Skip straight to step 3 unless we get genuine user cards.
                        var $userCards = $items.filter(function(){
                            // Real search-result item: has data-user-id OR
                            // an id starting with "user_" OR contains a like button.
                            var $i = $(this);
                            return $i.attr('data-user-id')
                                || ($i.attr('id') || '').indexOf('user_') === 0
                                || $i.find('[data-user-id], .btn_like, .icon_like, a[href*="profile_view"]').length > 0;
                        });
