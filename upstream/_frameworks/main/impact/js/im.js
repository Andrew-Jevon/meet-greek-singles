var CIm = function(guid, imHistoryMessages, isSp) {
    ...
    this.openIm = function(uid,e){
        ...
        if(e&&$targ.is('.im_title_name')&&$targ.closest('.open_im_chat.to_show')[0]){
    ...
    this.initOneIm = function(uid, firstMsgId){
        ...
        $(function(){
            ...
            $('.sngl_sb').on('click', function(e) {
                e.preventDefault();
                // Find the textarea associated with this UID and pass the DOM element to send()
                var textarea = $this.listImText[uid][0]; 
                $this.send(textarea);
            });
        ...
        return  $('<div class="bl_im_msg_one">'+
                        '<div class="bl_msg_check">'+
                            '<div class="top_msg_check">'+
                                '<div class="msg_check im_msg_user msg_arrow" data-date="'+l('just_now')+'" title=""><div class="p"></div>'+
    ...
    this.send = function(el){
        ...
        var uid=$el.data('uid');
        if($this.processUploadImage[uid])return false;
		if(!text && !$this.isImageLoaded[uid])return false;

        var $msg=$this.getTemplateMsg();
        if($this.isImageLoaded[uid]){
            $msg.find('.p').html('<div class="mod_image_preload">...</div>' + '\n');
        }
        ...
        $.post(url_main+'messages.php?cmd=send_message&display='+isOneChat,
              ...
              image_upload:$this.isImageLoaded[uid]?1:0,ind:$this.isImageLoaded[uid]},
              function(res){
                var data=checkDataAjax(res);
                if(data!==false){
                    data=$($.trim(data));
    ...
    this.showOriginalMessage = function($el){
        $el.slideUp(300);
        $el.next('.original_message').slideDown({duration: 300, step:function(){
            var uid=$el.closest('.open_im_chat').data('uid'),
                scrollPl=$this.uploadMsgScrollPl[uid];
    ...
    this.isImageLoaded={};
    this.clearUploadImage = function(uid, reset, checkPayd){
        if (checkPayd||false) {
            if($this.checkPaydUploadImage())return;
        }
        var $bl=$('#pp_message_upload_img_'+uid);
        if (reset||false) {
            if ($bl.is('.disabled')) {
                return;
            }
        ...
        $this.processUploadImage[uid]=false;
        $this.isImageLoaded[uid]=0;
        $this.addImageIm[uid].removeClass('to_hide');
        $this.addImageImLoader[uid].removeChildrenLoader();
        if(userAllowedFeature['upload_image_chat_paid']){
            $this.clickUpload($bl.find('.pp_message_upload_img_input_file'));
        }
    }

    this.initCheckPaydUploadImage = function($image){
        if(userAllowedFeature['upload_image_chat_paid']){
            if ($image.is('.no_available')) {
                $image.removeClass('no_available');
            }

            return false;
        } else if(!$image.is('.no_available')) {
            $image.addClass('no_available');
            return true;
        }
    }

    this.addImageIm={};
    this.addImageImLoader={};
    this.initAddImage = function(uid){
        $this.processUploadImage[uid]=false;
        $this.isImageLoaded[uid]=0;
        $this.addImageIm[uid]=$('#pp_message_upload_img_'+uid);
        $this.addImageImLoader[uid]=$('#pp_message_upload_img_loader_'+uid);

        $this.initCheckPaydUploadImage($this.addImageIm[uid]);

        $('#pp_message_upload_img_frm_'+uid).submit(function(e){
            var indx=+new Date,
                ...
                $this.processUploadImage[uid]=true;
                $this.addImageImLoader[uid].addChildrenLoader();
                $this.addImageIm[uid].addClass('to_hide').attr('title', '');
                        ...
                        if(xhr.status == 200) {
                            ...
                            if (data) {
                                if (data!='complete_upload_image') {
                                    ...
                                    $this.isImageLoaded[uid]=indx;

                                    $this.addImageIm[uid].removeClass('disabled')
                            ...
                            //$this.send(frm.closest('.foot').find('textarea')[0]);
                        }
                    }
                };
                xhr.send(formData);
