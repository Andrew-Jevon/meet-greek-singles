...
(function (CdvPurchase) {
    ...
    function log(verbosity, level, prefix, o) {
        ...
        if(o.indexOf('AppStore receipts loaded') !== -1) {
            o = 'AppStore receipts loaded custom';
            //console.trace();
        }
            ...
            runValidatorRequest(target, receipt, body, callback) {
                ...
                CdvPurchase.Utils.ajax(this.log.child("Ajax"), {
                    ...
                    success: (response) => {
                        ...
                        // Add custom fields from response for transaction finish
                        console.log('CdvPurchase.Utils.ajax', response);
                        var customFields = ['customTransactionId', 'customOriginalTransactionId', 'customProductId', 'customTransactionsInReceipt', 'customTransactions'];
                        for(var customFieldIndex in customFields) {
                            var customField = customFields[customFieldIndex];
                            var customFieldValue = response.data[customField];
                            if(customFieldValue) {
                                receipt[customField] = customFieldValue;
                                console.log('CdvPurchase.Utils.ajax', customField, customFieldValue);
                            }
                        }
