...
var BannerAd = /** @class */ (function (_super) {
    ...
    BannerAd.isLoaded = function (id) {
		//return execAsync(NativeActions.adIsLoaded, [{ id: id }]);
        return execAsync("adIsLoaded", [{ id: id }]);
    };
...
var WebViewAd = /** @class */ (function (_super) {
    ...
    WebViewAd.prototype.addAd = function (options) {
        var opts = __assign({ format: "auto", fullWidth: true }, options);
        if (this._loaded) {
            var html = opts.html || "";
            if (!opts.html) {
    ...
    WebViewAd.prototype.historySetPage = function (page, parameters) {
        ...
        for (var name_1 in parameters) {
            _parameters.push("".concat(name_1, "=").concat(encodeURI(parameters[name_1])));
        }
        var url = "".concat(page).concat(_parameters.length > 0 ? "?".concat(_parameters.join("&")) : "");
