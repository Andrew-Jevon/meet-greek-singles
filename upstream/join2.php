...
class CJoin2 extends UserFields//CHtmlBlock
{
	function action()
	{
        ...
        if ($isAjaxRequest) {
            ...
            if ($cmd == 'photo_upload') {
                ...
                // M4: server-side enforcement of Terms & Privacy acceptance.
                // Client-side checkbox can be bypassed via dev tools — this stops
                // account creation unless privacy_policy=1 is actually in the POST.
                $agreed = (get_param('privacy_policy') == '1')
                       || (get_param('agree') == '1')
                       || (get_session('j_privacy_policy') == '1');
                if (!$agreed) {
                    $responseData = $this->getResponseData('error_terms', 'You must agree to the Terms and Privacy Policy before creating an account.');
                    die(getResponseDataAjax($responseData));
                }
                // Remember acceptance for session (logged timestamp would be nicer, but keeping it minimal)
                ...
                if ($responseData) {
                    ...
                    if (!$uid) {
                        ...
                        set_session('social_photo_uploaded_name', '');
                        $g_user['user_id'] = $uid;
                        ...
                        if (Common::isOptionActive('manual_user_approval')) {
                            ...
                            // M3: send new accounts to the post-registration onboarding flow
                            // instead of straight to home. onboarding.php sets
                            // user.onboarding_done = 1 on submit (or skip), so this only
                            // catches genuinely new accounts.
                            $home   = Common::getHomePage();
                            $target = (defined('SITE_URL') ? SITE_URL : '') . 'onboarding.php';
                            // Fall back to onboarding.php at site root — Chameleon's
                            // routing resolves it correctly under /staging/ too.
                            $responseData = $this->getResponseData('redirect', 'onboarding.php');
                        }
                    }
                }else{
        ...
        $socialLoginPhoto = get_session('social_photo');
        if($socialLoginPhoto && !get_session('social_photo_uploaded_name')) {
            $name = 'tmp_join_impact_' . time() . '_' . mt_rand() . '_';
            $saveFile = $g['path']['dir_files'] . 'temp/' . $name;
            $im = new Image();
            if ($im->loadImage($socialLoginPhoto)) {
                $im->resizeCropped($g['image']['medium_x'], $g['image']['medium_y'], $g['image']['logo'], 0);
                $im->saveImage($saveFile . 'm.jpg', $g['image']['quality']);
                @chmod($saveFile . 'm.jpg', 0777);
                @file_put_contents($saveFile . 'src.jpg', urlGetContents($socialLoginPhoto));
                @chmod($saveFile . 'src.jpg', 0777);
                set_session('social_photo_uploaded_name', $name);
            }
        }
	...
	function parseBlock(&$html)
	{
        ...
        if ($isCustomRegister) {
            ...
            $socialPhotoUploadedName = get_session('social_photo_uploaded_name');
            if($socialPhotoUploadedName) {
                $isJoinWithPhotoOnly = false;
                $html->setvar('social_photo_uploaded_name', $socialPhotoUploadedName);
                $html->setvar('social_photo_uploaded_url', Common::getOption('url_files', 'path') . '/temp/' . $socialPhotoUploadedName . 'm.jpg');
                $html->parse('photo_upload_social');
            } else {
                $html->parse('photo_upload');
            }
