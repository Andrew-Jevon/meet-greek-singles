    ...
    function parseBlock(&$html)
    {
        ...
        if ($uid) {
            if ($groupId) {
                ...
                if ($groupsPhotoList){
                    ...
                    if($private) {
                        $pageTitle = l('your_private_photos');
                    }
                }
            } else {
                $name = User::getInfoBasic($uid, 'name');
                $name = User::nameShort($name);
                $pageTitle = lSetVars($private ? 'page_title_someones_private' : 'page_title_someones', array('name' => $name));
        ...
        if ($uid) {
            if ($groupsPhotoList){
                ...
                $pageUrl = Common::pageUrl($private ? 'user_photos_private_list' : 'user_photos_list', $uid);
            }
        } elseif ($groupsPhotoList){
        ...
        $itemsTotal = CProfilePhoto::getTotalPhotos($uid, false, $groupId, $private);
        ...
        if (!$ajax) {
            ...
            if ($guid) {
                if ($groupsPhotoList) {
                    ...
                    if (!$uid || $uid == $guid) {
                        if($private) {
                            $html->parse('page_link_photo_upload_private');
                        }
                        $html->parse('page_link_photo_upload', false);
                        $html->parse('page_link_photo_public_delimiter');
                        $html->parse('page_link_photo_private_delimiter');
                        if (!$uid) {
                    ...
                    if($uid && !$groupId) {
                        if($private) {
                            $html->parse('page_link_photo_public');
                        } else {
                            $html->parse('page_link_photo_private');
                        }
                    }
        ...
        CProfilePhoto::$isGetDataWithFilter = true;
        $rows = $class::parseListPhotos($html, $typeOrder, $limit, $groupId, true, $private);
        CProfilePhoto::$isGetDataWithFilter = false;
