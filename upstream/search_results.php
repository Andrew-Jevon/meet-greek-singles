...
if (!guid() && $optionTmplName == 'edge') {
    if (Common::isOptionActive('list_people_hide_from_guests', "{$optionTmplName}_general_settings") || ($display === 'profile' && Common::isOptionActive('list_people_profile_page_hide_from_guests', "{$optionTmplName}_general_settings"))) {
        Common::toLoginPage();
    }
}
...
//                        }
        ...
        $paramIamHereTo = get_param('i_am_here_to', UsersFilter::getVisitorFilterValue('i_am_here_to'));
        if(!$paramIamHereTo) {
        ...
        if (!get_param('set_filter')) {
            ...
            $userinfo['user_search_filters'] = UsersFilter::loadVisitorFilter($userinfo['user_search_filters']);
            User::setGetParamsFilter('user_search_filters', $userinfo);
...
$isEdgeLmsMode = Common::isEdgeLmsMode();
if($isEdgeLmsMode) {
    $where .= ' AND u.lms_user_type IN(' . LMS::getTeacherTypeIds() . ')';
}

if ($user["p_orientation"] > 0 && !$isEdgeLmsMode)
{
...
// 2026-05-01: free-text city filter (userinfo.home_city is the user-typed
// city captured at registration via the home_city user_var). Substring
// match so partial input narrows results — e.g. "Athens" matches "Athens"
// and "West Athens".
$user['home_city'] = trim((string) get_param('home_city', ''));
if ($user['home_city'] !== '')
{
    $hc = mb_substr($user['home_city'], 0, 60, 'UTF-8');
    $where .= " AND i.home_city LIKE '%" . to_sql($hc, 'Plain') . "%'";
}
...
//            }
            ...
            if($valFieldFrom || $valFieldTo) {
                ...
                if (!empty($params)){
                    ...
                    $from_add .= " LEFT JOIN users_checkbox AS " . $nameTable . " ON " . $nameTable . ".user_id = u.user_id AND " . $nameTable . ".field = " . to_sql($v['id'], 'Number') . " AND "  . $nameTable . ".value IN (" . to_sql_array($params) . ")";
...
if ($peopleNearby) {
    ...
    if ($guid) {
        ...
        $geoCityInfo = getDemoCapitalCountry();
...
if ($g_user['user_id']) {
    ...
    if ($display == 'encounters') {
        ...
        $where .=" AND u.is_photo_public = 'Y'
                   AND u.user_id != " . $guidSql .
                 ' AND upr.photo_id IS NULL ';
        $from_add .= ' LEFT JOIN photo AS up ON u.user_id = up.user_id AND up.private = "N" AND up.group_id = 0 ' . CProfilePhoto::wherePhotoIsVisible('up') . '
                       LEFT JOIN photo_rate AS upr ON up.photo_id = upr.photo_id AND upr.user_id = ' . $guidSql;
...
if (true) {
    ...
    if(($userSearchFiltersUpdate || $isResetFilter) && $userSearchFilters) {
        ...
        //echo '<br><br>FILTER>' . $filter . '<br><br>';
        // update if new only
        if(guid() && guser('user_search_filters') != $filter) {
            ...
            UsersFilter::saveVisitorFilter($filter);
        }
